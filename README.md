...
- 子 agent 的授权以根会话里用户的话为准（通过父会话的 `invoke_subagent` 记录回溯），找不到父会话时按不可信处理。回溯到的「根」本身也要像是人发起的会话（CLI 下：harness 写的设置快照），否则同样按不可信——嵌套子 agent 的顶层祖先找不到时，回溯会停在中间那个子 agent 上，而它的「用户消息」是 agent 写的。查找只看最近的 40 个会话，autoagy 自己的审核会话不占这个名额；名额不够看完时的「没找到」不会被缓存，下一次审核会重新找。
