...
| `writableRoots` | `[]` | 额外允许免审编辑的工作区外目录。因为 setup 会设 `allowNonWorkspaceAccess: false`，这里的每一项都需要一条 `write_file(<绝对路径>)` 授权，由 `autoagy setup` 一并写入——实测该授权覆盖整棵子树、不外溢到兄弟目录、并按解析后的落点判（目录里塞个指向外面的符号链接也逃不出去）。**改了这一项必须重跑 `autoagy setup`**：授权是 agy 启动时读一次的，没有对应授权的目录会被 agy 直接拒绝，无论 autoagy 怎么判。`autoagy status` 会检查这种不一致 |
