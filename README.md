...
| `protectedPaths` | `[]` | 额外需要审核才能修改的路径（glob）。**要么写绝对路径，要么用 `**/` 开头**（`.husky/**` 这种相对写法永远匹配不到绝对路径，会被丢弃并告警；`**/.husky/**` 可以，它**也匹配 `.husky` 目录本身**，所以 `rm -rf .husky` 同样送审）。编辑工具按解析后的落点判；**命令里字面写出这个路径的也送审**（`echo x > .husky/pre-commit`）——这是它存在的理由，因为「写了之后在沙箱外执行」的那一类（`.husky/`、`.envrc`、`postinstall`）正是命令写的。辨认是尽力而为，和命令分析的其它部分一样：运行时拼出来的路径（`$d/x`）找不到，`make` 这种读自己配置文件的工具也不会被这条拦下 |
