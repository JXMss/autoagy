...
- 只读：其余所有路径，其中包括工作区里的 `.git`、`.agents`、`.gemini` 等目录，autoagy 自己的目录，以及对话日志；**工作区里嵌套仓库（submodule、vendored checkout）的 `.git` 同样只读**——植在那里的 hook 会被下一条在那个目录里运行的 git 命令执行，而写操作的 git 命令必须离开沙箱才能成功，也就是在沙箱外执行。查找是有预算的广度优先扫描（最深 5 层、最多 1500 个目录，跳过 `node_modules`、`target`、`.venv` 等），埋得更深的仓库找不到——找不到只是回到没有这条之前的状态，不会影响其它保护；但**命令自己造出来的 `.git` 挂不上**：`--ro-bind-try` 会跳过构建命令行时还不存在的路径，而占位机制只覆盖顶层那几个名字，所以那一步只能事后发现。命令跑完后若出现新的嵌套 `.git` 且里面有会执行的东西——`hooks/` 下非 `.sample` 的文件，或 `config` 里让 git 执行命令的键（`hooksPath`、`sshCommand`、`pager`、`helper`、`clean`/`smudge`、`textconv`、`templateDir`……一整套，记录里写出命中的是哪个键），或 `[alias]`、`[include]`、`[includeIf]` 这几个 section（`[include]` 会让 git 去读另一个**不在 `.git` 里**的配置文件，所以按键列举追不上它）——就记一条 `planted-git-hook`、在 stderr 上告警，并且**之后触碰那个仓库的命令一律送审**（审核材料里带上 hook 内容），直到 `autoagy trust`。只看 `git init` 留下的空仓库是有意的：造仓库是日常操作，而合法的 husky／pre-commit／lefthook 安装在新造的嵌套仓库里与植入是同一个形状——这正是响应是"送审"而不是"把整个会话标记为不可信"的原因。
